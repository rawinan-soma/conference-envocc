...
**Profile population (resolves OQ-3):** The profile is completed by the user through a **self-service profile screen presented on first login**. The user cannot reach the main application until the required fields are complete. **Email is sourced from the organization's identity provider (FR-090) via the OIDC claim and is read-only** — the user cannot edit it. All other fields (title, first name, last name, phone, organization) are user-entered and app-owned (editable later by the user).
