...
describe('Story 1.7 — env.ts: validateEnv() (ATDD Red Phase)', () => {
	...
	// -----------------------------------------------------------------------
	test('[P0] 1.7-UNIT-001 — valid env with DATABASE_URL passes without calling process.exit', async () => {
	...
	// -----------------------------------------------------------------------
	test('[P0] 1.7-UNIT-002 — missing DATABASE_URL calls process.exit(1)', async () => {
		// THIS TEST WILL FAIL — src/lib/server/env.ts does not exist yet.
		// Activate after Task 6 (create validateEnv in src/lib/server/env.ts).
		const { validateEnv } = await import('./env.js');
	...
	// -----------------------------------------------------------------------
	test('[P0] 1.7-UNIT-003 — empty string DATABASE_URL calls process.exit(1)', async () => {
		// THIS TEST WILL FAIL — src/lib/server/env.ts does not exist yet.
	...
	// -----------------------------------------------------------------------
	test('[P1] 1.7-UNIT-004 — PORT and HOST are optional (default values applied)', async () => {
		// THIS TEST WILL FAIL — src/lib/server/env.ts does not exist yet.
		// Activate after Task 6.
		const { validateEnv } = await import('./env.js');
	...
	// -----------------------------------------------------------------------
	test('[P1] 1.7-UNIT-005 — completely empty env calls process.exit(1)', async () => {
		// THIS TEST WILL FAIL — src/lib/server/env.ts does not exist yet.
		// Activate after Task 6.
		const { validateEnv } = await import('./env.js');
	...
	// -----------------------------------------------------------------------
	test('[P1] 1.7-UNIT-006 — error message contains [startup] prefix for clarity', async () => {
		// THIS TEST WILL FAIL — src/lib/server/env.ts does not exist yet.
		// Activate after Task 6.
		const { validateEnv } = await import('./env.js');
