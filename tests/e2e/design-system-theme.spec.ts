...
test.describe('Story 1.2 — Design System & Thai Typography E2E (ATDD Red Phase)', () => {
	test('[P0] 1.2-E2E-001 — CSS custom properties --primary and --background resolve to Forest & Copper values', async ({
		...
		expect(primaryColor.toLowerCase(), '--primary must resolve to #2D6A4F (green-700)').toMatch(
			/#2d6a4f|#2D6A4F|var\(--green-700\)/i
		);

		// AC-1: --background → cream (#FAFAF7)
		expect(backgroundColor.toLowerCase(), '--background must resolve to #FAFAF7 (cream)').toMatch(
			/#fafaf7|var\(--cream\)/i
		);
		...
		// AC-1: --border → #E0DBD3
		expect(borderColor.toLowerCase(), '--border must resolve to #E0DBD3').toMatch(
			/#e0dbd3|var\(--border-color\)/i
		);
	});
	...
	test('[P0] 1.2-E2E-003 — page document has Google Fonts preconnect links in <head>', async ({
		...
		const preconnectGoogleapis = await page
			.locator('link[rel="preconnect"][href="https://fonts.googleapis.com"]')
			.count();
		expect(
		...
		const preconnectGstatic = await page
			.locator('link[rel="preconnect"][href="https://fonts.gstatic.com"]')
			.count();
		expect(preconnectGstatic, 'Must have preconnect link for fonts.gstatic.com').toBeGreaterThan(0);

		// Check for Google Fonts stylesheet link referencing Noto fonts
		const fontsStylesheet = await page.locator('link[rel="stylesheet"][href*="Noto"]').count();
		expect(
			fontsStylesheet,
			'Must have Google Fonts stylesheet link for Noto fonts'
		).toBeGreaterThan(0);
	});
