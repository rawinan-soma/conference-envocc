...
test.describe('Story 2.1 — OIDC Sign-in Flow (AC-2, AC-3)', () => {
	test.skip(
		'[P0] 2.1-E2E-001 — OIDC authorization-code + PKCE flow completes → DB-backed session created',
		async ({ page }) => {
			...
			await page.goto('/dashboard');
			await expect(page, 'Authenticated visit to /dashboard must NOT redirect to /login').not.toHaveURL(
				/\/login/
			);
...
test.describe('Story 2.1 — Login Page: Render and Interaction (AC-2)', () => {
	test.skip(
		'[P0] 2.1-E2E-LOGIN-001 — Login page renders with Sign in button',
		async ({ page }) => {
	...
	test.skip(
		'[P0] 2.1-E2E-LOGIN-002 — Unauthenticated visit to (app) route redirects to login page',
		async ({ page }) => {
	...
	test.skip(
		'[P0] 2.1-E2E-LOGIN-003 — Clicking Sign in button initiates OIDC redirect',
		async ({ page }) => {
			// THIS TEST WILL FAIL — login page and auth flow not yet implemented.
...
test.describe('Story 2.1 — Login Page: i18n and Thai Language (AC-2, NFR-006)', () => {
	test.skip(
		'[P1] 2.1-E2E-002 — Login page renders with Paraglide i18n and html[lang] attribute',
		async ({ page }) => {
			...
			expect(
				htmlLang,
				'html[lang] attribute must be set by Paraglide (either "en" or "th")'
			).toMatch(/^(en|th)$/);
	...
	test.skip(
		'[P1] 2.1-E2E-002b — Login page has no hardcoded English or Thai strings (all via Paraglide)',
		async ({ page }) => {
			// THIS TEST WILL FAIL — login page not yet created (Task 3.3).
...
test.describe('Story 2.1 — Login Redirect: Preserve Original URL (UX)', () => {
	test.skip(
		'[P1] 2.1-E2E-003 — Login redirect preserves originally-requested URL after authentication',
		async ({ page }) => {
			...
			expect(
				bypassResponse.status(),
				'Dev bypass must succeed — activate after Story 2.2'
			).toBe(200);
...
test.describe('Story 2.1 — Login Page: OIDC Provider Error Handling [P2]', () => {
	test.skip(
		'[P2] 2.1-E2E-004 — Login page shows actionable error message when OIDC provider is unavailable',
		async ({ page }) => {
...
test.describe('Story 2.1 — Accessibility: Login Page (NFR-007)', () => {
	test.skip(
		'[P1] 2.1-E2E-A11Y — Login page passes axe-core WCAG 2.1 AA check (zero violations)',
		async ({ page }) => {
			// THIS TEST WILL FAIL — login page not yet created (Task 3.3).
