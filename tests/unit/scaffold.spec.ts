...
describe('Story 1.1 — Scaffold Acceptance Tests (ATDD Red Phase)', () => {
  ...
  test.skip('[P1] 1.1-UNIT-001 — bun install succeeds (exit 0)', () => {
    ...
    // After `bunx sv create .` + `bun install` complete, remove test.skip() to activate.
  ...
  test.skip('[P1] 1.1-UNIT-001b — package.json has all required scripts', () => {
    ...
    const requiredScripts = ['dev', 'build', 'preview', 'check', 'lint', 'format', 'test', 'test:e2e'];
  ...
  test.skip('[P1] 1.1-UNIT-002 — bun run build produces Bun server bundle (exit 0)', () => {
    ...
    // Remove test.skip() after sv create + svelte-adapter-bun are configured.
    ...
    expect(existsSync(serverEntry), 'svelte-adapter-bun server bundle not found at build/index.js').toBe(true);
  });
  ...
  test.skip('[P1] 1.1-UNIT-002b — svelte.config.js uses svelte-adapter-bun (not adapter-node)', () => {
    // THIS TEST WILL FAIL — svelte.config.js does not exist yet.
    const configPath = path.join(PROJECT_ROOT, 'svelte.config.js');

    expect(existsSync(configPath), 'svelte.config.js not found').toBe(true);
  ...
  test.skip('[P2] 1.1-UNIT-004 — svelte-adapter-bun produces standalone Bun server bundle', () => {
    ...
    expect(existsSync(serverEntry), 'build/index.js not found — run `bun run build` first').toBe(true);
    ...
    expect(entryContent, 'build/index.js appears to be Node adapter output (http.createServer found without Bun.serve)').toSatisfy(
  ...
  test.skip('[P1] 1.1-UNIT-001e — vite.config.ts includes Tailwind v4 and Paraglide Vite plugins', () => {
    ...
    expect(content, 'Tailwind v4 Vite plugin not found in vite.config.ts').toMatch(/@tailwindcss\/vite/);

    // Paraglide Vite plugin
    expect(content, 'Paraglide Vite plugin not found in vite.config.ts').toMatch(/paraglide/i);
  });

  test.skip('[P1] 1.1-UNIT-001f — hooks.server.ts wires Paraglide i18n.handle()', () => {
    // THIS TEST WILL FAIL — hooks.server.ts not yet created.
    ...
    expect(content, 'Paraglide i18n import missing from hooks.server.ts').toMatch(/i18n/);
    expect(content, 'i18n.handle() not found in hooks.server.ts').toMatch(/i18n\.handle\(\)/);
  });

  test.skip('[P1] 1.1-UNIT-001g — project.inlang/settings.json has en source and th locale', () => {
    // THIS TEST WILL FAIL — project.inlang/ not yet created.
    ...
    const settings = JSON.parse(readFileSync(settingsPath, 'utf-8')) as { sourceLanguageTag?: string; languageTags?: string[] };

    expect(settings.sourceLanguageTag, 'sourceLanguageTag must be "en"').toBe('en');
    expect(settings.languageTags, 'languageTags must include "th"').toContain('th');
  });
  ...
  test.skip('[P1] 1.1-UNIT-001h — drizzle.config.ts uses node-postgres dialect and correct schema path', () => {
    ...
    expect(content, 'drizzle.config.ts must use dialect: postgresql').toMatch(/dialect.*postgresql/);
    expect(content, 'drizzle.config.ts must reference schema in src/lib/server/db/').toMatch(/src\/lib\/server\/db\/schema/);
  ...
  test.skip('[P2] 1.1-UNIT-003e — no hardcoded Tailwind config.js file exists (Tailwind v4 is CSS-only)', () => {
    ...
    expect(existsSync(legacyConfig), 'tailwind.config.js must NOT exist — Tailwind v4 uses CSS-only config').toBe(false);
