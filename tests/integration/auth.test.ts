...
describe('Story 2.1 — Auth Guard: Unauthenticated Redirect (AC-1, AC-5)', () => {
	test.skip('[P0] 2.1-INT-001 — Unauthenticated GET to (app) route redirects 302 to /login', async () => {
		...
		expect(location, '302 redirect must point to /login (the Better Auth entry point)').toMatch(
			/\/login/
		);
	});

	test.skip('[P0] 2.1-INT-001b — Unauthenticated GET to nested (app) route also redirects 302', async () => {
		...
		expect(response.status, 'Unauthenticated GET to /bookings must return 302').toBe(302);

		const location = response.headers.get('location');
		expect(location, '302 redirect must point to /login').toMatch(/\/login/);
	});

	test.skip('[P1] 2.1-INT-001c — Public route /login is accessible without session (no redirect loop)', async () => {
	...
	test.skip('[P1] 2.1-INT-001d — Public route /auth/** is accessible without session (OIDC callback receiver)', async () => {
...
describe('Story 2.1 — Session Lifecycle: Logout Destroys Session (AC-4)', () => {
	test.skip('[P0] 2.1-INT-002 — POST /auth/sign-out destroys DB session row; subsequent (app) GET → 302', async () => {
		...
		try {
			...
			expect(appResponse.status, 'Subsequent (app) GET with deleted session must return 302').toBe(
				302
			);
...
describe('Story 2.1 — OIDC Callback Safety: No code/state Leak (AC-6)', () => {
	test.skip('[P1] 2.1-INT-003 — OIDC callback response does not echo code or state parameters', async () => {
		// THIS TEST WILL FAIL — auth route not yet created (Task 3.1).
		...
		expect(response.status, 'OIDC callback with invalid params must not return 5xx').toBeLessThan(
			500
		);
		...
		expect(responseBody, 'Response body must not echo the OIDC state parameter').not.toContain(
			fakeState
		);
...
describe('Story 2.1 — OIDC Error Handling: Invalid State (AC-6, UX)', () => {
	test.skip('[P2] 2.1-INT-004 — OIDC callback with invalid state returns user-facing error, not 500', async () => {
		// THIS TEST WILL FAIL — auth route not yet created (Task 3.1).
		...
		expect(response.status, 'OIDC callback with invalid state must return 4xx or 3xx').toBeLessThan(
			500
		);
	});
});
...
describe('Story 2.1 — Auth Config: Fixed Session Timeout (FR-093)', () => {
	test('[P1] 2.1-UNIT-001 — Better Auth config has session.expiresIn fixed at 1800 seconds (30 min)', async () => {
		// ACTIVATED — src/lib/server/auth/index.ts created in Task 1.4.
		//
