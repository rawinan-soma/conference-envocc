...
describe('Story 4.8 — Dashboard Query: Organizer Scope + IDOR Boundary (AC-1, FR-050)', () => {
	test("[P0] 4.8-INT-001 — getUpcomingBookingsByOrganizer returns only the requesting organizer's upcoming active bookings; another organizer's bookings are absent", async () => {
...
describe('Story 4.8 — Dashboard Query: Cancelled Bookings Excluded (AC-1)', () => {
	test('[P1] 4.8-INT-002 — cancelled bookings are excluded from getUpcomingBookingsByOrganizer results', async () => {
		// THIS TEST WILL FAIL until getUpcomingBookingsByOrganizer() is implemented.
		...
		expect(eventNames, 'Active booking must appear in dashboard results').toContain(
			'4.8 Active Event INT-002'
		);
...
describe('Story 4.8 — Dashboard Query: Past Bookings Excluded (AC-1)', () => {
	test('[P1] 4.8-INT-003 — past bookings (upper(during) < now()) are excluded; only future bookings returned', async () => {
		// THIS TEST WILL FAIL until getUpcomingBookingsByOrganizer() is implemented.
		// Activate after 4.8-INT-001 passes.
		//
		...
		expect(eventNames, 'Future booking must appear in dashboard results').toContain(
			'4.8 Future Event INT-003'
		);
